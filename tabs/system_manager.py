...
class SystemManagerTab(TabModule):
    ...
    def _remove_game(self):
        rows = {i.row() for i in self.games_table.selectedIndexes()}
        if not rows:
            return
        names = [self._games_data[r]["name"] for r in rows if r < len(self._games_data)]
        reply = QMessageBox.question(
            self.parent, "Confirmar eliminación",
            f"¿Eliminar {len(names)} juego(s)?\n" + "\n".join(names[:10]),
